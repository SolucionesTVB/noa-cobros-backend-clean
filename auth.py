...
def bootstrap_admin():
    ...
    tok = create_access_token(identity={"u": getattr(u,"username",username), "r": getattr(u,"role","admin")}, expires_delta=timedelta(hours=12))
...
def login():
    ...
    tok = create_access_token(identity={"u": getattr(u,"username",username), "r": getattr(u,"role","client")}, expires_delta=timedelta(hours=12))
...
def admin_create_user():
    ...
    u = User()
    if hasattr(u,"username"): u.username = username
    if hasattr(u,"password_hash"): u.password_hash = generate_password_hash(pwd)
    elif hasattr(u,"password"):    u.password      = generate_password_hash(pwd)
    if hasattr(u,"role"): u.role = role if role in ("admin","tester","client") else "client"
