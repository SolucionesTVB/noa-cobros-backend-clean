...
def bootstrap_admin():
    ...
    exists = q.filter(getattr(User, "role", "") == "admin").first() if hasattr(User, 
"role") else None
    if exists:
    ...
    tok = create_access_token(
        identity={"u": getattr(u, "username", username), "r": getattr(u, "role", 
"admin")},
...
def admin_create_user():
    ...
    me = db.session.query(User).filter(getattr(User, "username") == ident.get("u", 
"")).first()
